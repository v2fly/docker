...
main() {
    ...
    if [ -z "${ARCH}" ]; then
        echo "Error: Not supported OS Architecture" && exit 1
    fi
    ...
    if [ $? -ne 0 ]; then
        echo "Error: Failed to download binary file: ${DGST_FILE}" && exit 1
    fi
...
# with "$@"
# to handle ./v2ray.sh "one two    three" four five "six seven"
# with main()
# now you can use it like: curl -L localhost/v2ray.sh | sh
